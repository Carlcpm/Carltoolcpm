...
def get_service_costs():
    ...
    return {
        ...
        "copy_cpm2_car_to_cpm2": 10000, # Add default cost for new service
        "unlock_all_flags": 25000, # Add default cost for new service 
        "unlock_police_kits": 35000, # Add default cost for new service 
        "register_vip_unlimited": 50000, # Add default cost for new service 
        "add_car": 10000, # Add default cost for new service 
    }
        ...
        while True:
            ...
            while True: # Submenu loop
                ...
                if main_menu == "2": # CPM2 specific options
                    ...
                    print(f"15. 🔄 COPY CPM1 CAR TO CPM2 (Cost: {service_costs.get('copy_cpm1_car_to_cpm2', 10000)} coins)") 
                    print(f"16. 🚗 COPY CPM2 CAR TO CPM2 (Cost: {service_costs.get('copy_cpm2_car_to_cpm2', 10000)} coins)")
                    print(f"17. 🎌 UNLOCK ALL FLAG (Cost: {service_costs.get('unlock_all_flags', 25000)} coins)")
                    print(f"18. 🚔 UNLOCK POLICE KITS (Cost: {service_costs.get('unlock_police_kits', 35000)} coins)")
                    print(f"19. 🆓 REGISTER VIP ACCOUNT (UNLIMITED ONLY) ( (Cost: {service_costs.get('register_vip_unlimited', 50000)} coins)")
                    print(f"20.➕ ADD CAR (Cost: {service_costs.get('add_car', 10000)} coins per car)")
                print("0. 🔙 BACK")
                ...
                elif choice == "15" and main_menu == "2": # COPY CPM1 CAR TO CPM2 
               ...
               elif choice == "16" and main_menu == "2": # COPY CPM2 CAR TO CPM2 
                    cpm1_email_input = input("📧 Enter CPM1 Email: ").strip()
                    cpm1_password_input = input("🔐 Enter CPM1 Password: ").strip()
                    action_result = call_php_service(access_key, "copy_cpm2_car_to_cpm2", token, email, password, {
                        "cpm2_email": cpm1_email_input,
                        "cpm2_password": cpm1_password_input,
                        "cpm2_api_key": api_key_cpm2 # Pass CPM2 API key for the function to                        
               elif choice == "17" and main_menu == "2": # UNLOCK ALL FLAG 
                    action_result = call_php_service(access_key, "unlock_flags", token, email, password)
               elif choice == "18" and main_menu == "2": # UNLOCK POLICE KITS 
                    action_result = call_php_service(access_key, "unlock_police_kits", token, email, password)
               elif choice == "19" and main_menu == "2": # REGISTER VIP ACCOUNT (UNLIMITED ONLY) 
                    action_result = call_php_service(access_key, "register_vip_unlimited", token, email, password)
               elif choice == "20" and main_menu == "2": # ADD CAR 
