...
def get_service_costs():
    ...
    "king_rank": 25000,
    "change_email": 25000,
    "change_password": 25000,
    "register_vip_account": 0,  # UNLIMITED ONLY
    "set_money": 50000,
    "unlock_wheels": 25000,
    "unlock_male": 25000,
    "unlock_female": 25000,
    "unlock_brakes": 25000,
    "unlock_calipers": 25000,
    "unlock_paints": 25000,
    "unlock_all_flags": 25000,
    "unlock_apartments": 25000,
    "complete_missions": 25000,
    "unlock_all_cars_siren": 50000,
    "unlock_police_kits": 25000,
    "unlock_slots": 50000,
    "copy_cpm1_car_to_cpm2": 100000,
    "clone_cars_cpm2_to_cpm2": 100000,
    "add_car": 10000
}           
    ...
    while True:
        ...
        while True:
            ...
            while True: # Submenu loop
                ...
                print(f"04. 🌟 REGISTER VIP ACCOUNT (Cost: {service_costs.get('register_vip_account', 0)} coins)")
                print(f"05. 🛞 UNLOCK WHEELS (Cost: {service_costs.get('unlock_wheels', 50000)} coins)")
                print(f"06. 💰 SET MONEY (Cost: {service_costs.get('set_money', 25000)} coins)")
                ...
                print(f"09. 🛠️ UNLOCK BRAKES (Cost: {service_costs.get('unlock_brakes', 25000)} coins)")
                print(f"10. 🛠️ UNLOCK CALIPERS (Cost: {service_costs.get('unlock_calipers', 25000)} coins)")
                print(f"11. 🎨 UNLOCK PAINTS (Cost: {service_costs.get('unlock_paints', 25000)} coins)")
                print(f"12. 🚩 UNLOCK ALL FLAGS (Cost: {service_costs.get('unlock_all_flags', 25000)} coins)")
                print(f"13. 🏠 UNLOCK APARTMENTS (Cost: {service_costs.get('unlock_apartments', 25000)} coins)")
                print(f"14. 💯 COMPLETE MISSIONS (Cost: {service_costs.get('complete_missions', 25000)} coins)")
                print(f"15. 🚨 UNLOCK SIREN & AIRSUS (Cost: {service_costs.get('unlock_all_cars_siren', 50000)} coins)")
                print(f"16. 🚓 UNLOCK POLICE KITS (Cost: {service_costs.get('unlock_police_kits', 25000)} coins)")
                print(f"17. 📦 UNLOCK SLOTS (Cost: {service_costs.get('unlock_slots', 50000)} coins)")
                print(f"18. 🔄 CLONE CARS CPM1 TO CPM2 (Cost: {service_costs.get('copy_cpm1_car_to_cpm2', 100000)} coins)")
                print(f"19. 🚗 CLONE CARS CPM2 TO CPM2 (Cost: {service_costs.get('clone_cars_cpm2_to_cpm2', 100000)} coins)")
...
elif choice == "4" and main_menu == "2":  # REGISTER VIP ACCOUNT (CPM2 only)
    vip_code = input("🆔 Enter VIP code: ").strip()
    action_result = call_php_service(access_key, "register_vip", token, email, password, {
        "vip_code": vip_code
    })

elif choice == "5" and main_menu == "2":  # SET MONEY (CPM2 only)
    amount = input("💵 Enter amount to set: ").strip()
    if amount.isdigit():
...
elif choice == "6" and main_menu == "2":  # UNLOCK WHEELS (CPM2 only)
    action_result = call_php_service(access_key, "unlock_wheels", token, email, password)

elif choice == "7" and main_menu == "2":  # UNLOCK MALE (CPM2 only)
    action_result = call_php_service(access_key, "unlock_male", token, email, password)

elif choice == "8" and main_menu == "2":  # UNLOCK FEMALE (CPM2 only)
    action_result = call_php_service(access_key, "unlock_female", token, email, password)

elif choice == "9" and main_menu == "2":  # UNLOCK BRAKES (CPM2 only)
    action_result = call_php_service(access_key, "unlock_brakes", token, email, password)

elif choice == "10" and main_menu == "2":  # UNLOCK CALIPERS (CPM2 only)
    action_result = call_php_service(access_key, "unlock_calipers", token, email, password)

elif choice == "11" and main_menu == "2": # UNLOCK PAINTS (CPM2 only)
    action_result = call_php_service(access_key, "unlock_paints", token, email, password)

elif choice == "12" and main_menu == "2":  # UNLOCK ALL FLAGS (CPM2 only)
    action_result = call_php_service(access_key, "unlock_flags", token, email, password)

elif choice == "13" and main_menu == "2":  # UNLOCK APARTMENTS (CPM2 only)
    action_result = call_php_service(access_key, "unlock_apartments", token, email, password)

elif choice == "14" and main_menu == "2":  # COMPLETE MISSIONS (CPM2 only)
    action_result = call_php_service(access_key, "complete_missions", token, email, password)

elif choice == "15" and main_menu == "2":  # UNLOCK SIREN & AIRSUS (CPM2 only)
    action_result = call_php_service(access_key, "unlock_all_cars_siren", token, email, password)

elif choice == "16" and main_menu == "2":  # UNLOCK POLICE KITS (CPM2 only)
    action_result = call_php_service(access_key, "unlock_police_kits", token, email, password)

elif choice == "17" and main_menu == "2":  # UNLOCK SLOTS (CPM2 only)
    action_result = call_php_service(access_key, "unlock_slots", token, email, password)

elif choice == "18" and main_menu == "2":  # CLONE CARS CPM1 TO CPM2 (CPM2 only)
    account_email_input    = input("📧 Enter CPM1 Account Email to clone to: ").strip()
    account_password_input = input("🔐 Enter CPM1 Account Password to clone to: ").strip()
    action_result = call_php_service(access_key, "clone_cars_cpm1_to_cpm2", token, email, password, {
        "account_email": account_email_input,
        "account_password": account_password_input
    })

elif choice == "19" and main_menu == "2":  # CLONE CARS CPM2 TO CPM2 (CPM2 only)
...
elif choice == "20" and main_menu == "2":  # ADD CAR (CPM2 only)
    car_id = input("🚗 Enter Car ID to add: ").strip()
