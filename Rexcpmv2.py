...
if os.path.basename(__file__) != 'Rexcpmv2.py':
    console.print("\n[bold red]⛔ Error:[/bold red] This file has been renamed.")
    console.print("[yellow]Please rename it back to[/yellow] [bold]Rexcpmv2.py[/bold] [yellow]to use this tool.[/yellow]\n")
    sys.exit(1)
...
def show_banner(unlimited_status=None, current_coins=None):
    banner_text = Text()
    banner_text.append("🚗  👑 REX CPM X CRIS TOOL 👑  🚗\n", style="bold cyan")
    banner_text.append("🔥 SUPPORTS CPM1 & CPM2 🔥\n", style="bold green")
    banner_text.append("🔑 Share Key Not Allowed 🚫\n", style="bold red")
    banner_text.append("🪙 Buy Credit: ", style="yellow")
    banner_text.append("@Rexcpm\n", style="bold magenta")

    if unlimited_status is not None:
        if unlimited_status:
            banner_text.append("✅ Subscription: UNLIMITED\n", style="bold green")
        else:
            banner_text.append("❌ Subscription: LIMITED\n", style="bold red")
            if current_coins is not None:
                banner_text.append(f"💰 Balance: {current_coins} coins\n", style="bold yellow")

    console.print(Panel(banner_text, title="[bold blue]REXCPM2 SYSTEM", subtitle="By REXCPM", border_style="bright_blue", padding=(1, 4)))

    spinner = Spinner("dots", text="Initializing Modules...", style="bold blue")
    with console.status(spinner, spinner="line", speed=0.5):
        time.sleep(2)  # Simulate loading time

    animated_typing("✔️ Tool Loaded Successfully!")
        ...
        while True:
            ...
            while True:
                ...
                if main_menu == "2":
                    ...
                    print(f"15. 🔄 CLONE CARS CPM1 TO CPM2 (Cost: {service_costs.get('copy_cpm1_car_to_cpm2', 10000)} coins)")
